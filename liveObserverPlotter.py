...
def drawField(plax):
    ...
    plax.plot([p[0] for p in platform], [p[1] for p in platform], color="grey")
    
    nullZoneLeft = [(0.0, 288.0), (95.25, 288.0), (95.25, 288.0+72.0), (0, 288.0+72.0)]
    plax.plot([p[0] for p in nullZoneLeft], [p[1] for p in nullZoneLeft], linestyle=':', color="grey")
    nullZoneRight = [(fieldDim[0], 288.0), (fieldDim[0]-95.25, 288.0), (fieldDim[0]-95.25, 288.0+72.0), (fieldDim[0], 288.0+72.0)]
    plax.plot([p[0] for p in nullZoneRight], [p[1] for p in nullZoneRight], linestyle=':', color="grey")
...
point, = ax.plot(xdata[0], ydata[0], marker='o', markersize=5, color="teal")
startingRobot = genRobotSquare((xdata[0], ydata[0]), 0.0)
robot, = ax.plot([p[0] for p in startingRobot], [p[1] for p in startingRobot],marker='8', color="#F6903D")
